...
The source of truth for requirements is `specs/{feature}/spec.md`. If execution planning discovers requirement-impacting changes, `spec.md` must be updated and reset to `draft` before implementation continues.

### IV. Execution design lives with the plan
Keep execution-facing design, task breakdown, and progress together in `specs/{feature}/plan.md` so backflow updates do not need a second execution document. Ambiguity resolution is handled inside `spec-specify`.
